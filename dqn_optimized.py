...
from dqn_environment_opt import TorTraffic
from collections import namedtuple
from collections import deque
import argparse
import numpy as np
from dqn_environment_opt import CombinedModel, ResNet18, BasicBlockDilated, AWF
...
EPSILON_DECAY_LAST_FRAME = 25000
EPSILON_START = 1.0
EPSILON_FINAL = .01
...
class ExperienceBuffer:
    ...
    def sample(self, batch_size):
        ...
        #problem: state is currently a tuple of information about state...maybe change to single array? see previous version
...
class Agent:
    ...
    def play_step(self, net, epsilon=0.0, device="cpu"):
        ...
        #get relevant states, but calculate reward later
        for i in range(50):
            if np.random.random() < epsilon:
                action = np.random.randint(0, self.env.action_size)
            else:
                direction, time, label = self.state
            ...
            new_state, is_done, website, self.current_trace = self.env.step(action)
        ...
        #get rewards and then add (s,a,r,s') to buffer
        rewards = self.env.get_rewards(state_list)
        #print("Mean reward: {}".format(np.mean(rewards)))

        buffer_state_list = []
        for state in state_list:
            direction, time, label = state
            state_vector = np.concatenate((direction, time))
            buffer_state_list.append(state_vector)


        for i in range(1,49):
            exp = Experience(buffer_state_list[i], action_list[i], rewards[i], done_list[i], buffer_state_list[i+1])
    ...
    while True:
        ...
        if reward is not None:
            ...
            if best_mean_reward is None or best_mean_reward < mean_reward:
                torch.save(net.state_dict(), "dqn_wf_opt_best.dat")
